...
describe('CRUD package', function () {
	...
	let secondUserName = 'second',
		secondUserId;
	let workbookIds = [];
	let initialPackageName = 'create it in before method';
	...
	before(async () => {
		try {
			...
			const secondRes = await agent.get(
				'/api/getuserbyusername/' + secondUserName
			);
			if (secondRes.body.user) {
				secondUserId = secondRes.body.user._id;
				secondUserName = secondRes.body.user.username;
			}
			...
			console.log('Current user is : ' + oneUserName + '  ' + oneUserId);
			console.log(
				'The other user is : ' + secondUserName + '   ' + secondUserId
			);

			await agent
				.post('/api/v2/admin/workbook')
				.send(require('../workbooks/workbook.json'))
				.then((res) => {
					console.log(res.body.message);
				});
			await agent
				.post('/api/v2/admin/workbook')
				.send(require('../workbooks/workbook02.json'))
				.then((res) => {
					console.log(res.body.message);
				});
			await agent
				.post('/api/v2/admin/workbook')
				.send(require('../workbooks/workbook03.json'))
				.then((res) => {
					console.log(res.body.message);
				});

			// Retrieve workbook
			const workbooksRes = await agent.get('/api/v2/admin/workbooks');
			const workbooks = workbooksRes.body.workbooks;
			for (let i = 0; i < 3 && i < workbooks.length; i++) {
				if (workbooks[i]) {
					workbookIds.push(workbooks[i]._id);
					console.log('Received workbooks: ' + workbooks[i].name);
				}
			}
	...
	xit('GET by user name and package name - success', async () => {
		// FIXME: method wrong
		this.timeout(10000);
		const urlStr =
			'/api/admin/' + secondUserName + '/packages/' + initialPackageName;
		try {
			const res = await agent.get(urlStr);
	...
	xit('GET by  package name - success', (done) => {
		this.timeout(10000);
		const urlStr =
			'/api/v2/admin/packages/' + initialPackageName + '/organizations';
		agent
			.get(urlStr)
			.then(function (res) {
				console.table(res.body);
	...
	xit('GET by  query attributeId and categroyId - success', (done) => {
		this.timeout(10000);
		const queryString =
			'?categoryId=' + 100679515 + '&attributeId=' + 100045567;
	...
	xit('GET by  query attributeId and categroyId - failed', (done) => {
		this.timeout(10000);
		const queryString =
			'?categoryId=' + 100679515666 + '&attributeId=' + 100045567666;
		const urlStr = '/api/admin/packages/' + initialPackageName + queryString;
		agent
			.get(urlStr)
			.then(function (res) {
	...
	xit('GET all by current user - success', (done) => {
		this.timeout(10000);
	...
	xit('GET by user name - success', (done) => {
		this.timeout(10000);
	...
	xit('GET by package name - Not Found', (done) => {
		this.timeout(10000);
	...
	xit('GET by package name - admin - Not Found', (done) => {
		this.timeout(10000);
		const urlStr = '/api/v2/admin/packages/1x1x1';
		agent
			.get(urlStr)
			.then(function (res) {
				console.table(res.body);
	...
	xit('Post - no end Date - failed', (done) => {
		this.timeout(10000);
	...
	xit('Post - start date less than end date - failed', (done) => {
		this.timeout(10000);
		const urlStr = '/api/admin/packages';
		agent
			.post(urlStr)
	...
	xit('Post - package already exists - failed', (done) => {
		this.timeout(10000);
		const urlStr = '/api/admin/packages';
		agent
			.post(urlStr)
	...
	xit('Post create second Package - success', (done) => {
		// name did not exist
		this.timeout(10000);
		const urlStr = '/api/admin/packages';
		agent
	...
	xit('Put - update the workbookIds - success', async () => {
		// name did not exist
		this.timeout(10000);
	...
	xit('Put  - error end date - failed', (done) => {
		// name did not exist
		this.timeout(10000);
	...
	xit('Put  - no attributeId - failed', (done) => {
		// name did not exist
		this.timeout(10000);
	...
	xit('Put  - /api/admin/packagevalues - success', async () => {
		// name did not exist
		this.timeout(10000);
