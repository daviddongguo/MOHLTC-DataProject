...
router.post('/api/admin/packages', async (req, res, next) => {
	...
	if (startDate >= endDate) {
		return res
			.status(400)
			.json({success: false, message: 'startDate must be less than endDate.'});
	}
	if (!name) {
		return res
			.status(400)
			.json({success: false, message: 'package must have a name.'});
	}
	if (!userIds) {
	...
	try {
		...
		dbWorkbooks = await Workbook.find({_id: {$in: workbookIds}})
			.populate('sheets')
			.exec();
		if (!dbWorkbooks) {
			return res
				.status(400)
				.json({success: false, message: 'dbWorkbooks do not exist'});
		} else {
	...
	if (!dbValues) {
		return res
			.status(400)
			.json({success: false, message: 'values do not exist'});
	}
	const data = {};
...
router.put('/api/admin/packages/:packagename', async (req, res, next) => {
	...
	if (!queryPackageName) {
		return res
			.status(400)
			.json({success: false, message: 'package name can not be empty.'});
	}
	const dbPackage = await Package.findOne({
	...
	if (
		(startDate && !endDate && startDate >= dbPackage.endDate) ||
	...
	if (false || newWorkbookIds[0]) {
		const newDbWorkbooks = await Workbook.find({_id: {$in: newWorkbookIds}})
			.populate('sheets')
			.exec();
		const rowIds = [];
		let columnIds = {};
	...
	if (false || deleteWorkbookIds[0]) {
		const deleteDbWorkbooks = await Workbook.find({
			_id: {$in: deleteWorkbookIds},
		})
			.populate('sheets')
			.exec();
		const rowIds = [];
	...
	try {
		...
		dbPackage.workbooks = queryDbWorkbookIds[0]
			? queryDbWorkbookIds
			: dbPackage.workbooks;
				...
				if (rowKey === queryCategoryId.toString()) {
					for (let columnKey in PackageValues.data[rowKey]) {
						if (columnKey === queryAttributeId.toString()) {
							dbPackage.values.data[queryCategoryId][
								queryAttributeId
							] = newValue;
							dbPackage.markModified('values.data');
							await dbPackage.save();
