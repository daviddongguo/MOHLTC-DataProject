...
async function userGetPackageAndWorkbook(
	next,
	currentUserId,
	groupNumber,
	packageName,
	...
	if (name) {
		...
		if (!workbook)
			return next({status: 400, message: 'Workbook does not exist.'});
	}
...
module.exports = {
	...
	adminGetPackage: async (req, res, next) => {
		...
		try {
			...
			if (!orgDoc)
				return next({
					status: 400,
					message: `Organizations (${organization}) does not exist.`,
				});

			// if the package is submitted, then the admin will get the package value.
			const packageValue = await PackageValue.findOne({
				groupNumber,
				package: pack._id,
				organization: orgDoc._id,
			});
			if (packageValue.histories.length > 0) {
				const {userNotes, submittedBy, date} = packageValue.histories[
					packageValue.histories.length - 1
				];
				const {
					workbooks,
					startDate,
				...
				const submittedUser = await User.findById(
					submittedBy,
	...
	userGetPackage: async (req, res, next) => {
		...
		const name = req.params.name; // package name
		try {
			const result = await userGetPackageAndWorkbook(
				next,
				currentUserId,
				groupNumber,
				name
	...
	adminGetAllPackages: async (req, res, next) => {
		...
		const groupNumber = req.session.user.groupNumber;
		try {
			const packages = await Package.find({groupNumber}).populate({
				path: 'organizations',
				select: 'name',
			});
	...
	adminDeletePackage: async (req, res, next) => {
		...
		try {
			...
			await PackageValue.deleteMany({
				package: dbPackage._id,
				organization: {$in: dbPackage.organizations},
			});
	...
	adminCreatePackage: async (req, res, next) => {
		...
		if (startDate > endDate) {
			return next({
				status: 400,
				message: 'startDate must be less or equal to endDate.',
			});
		}
		if (!name) {
		...
		try {
			...
			const valueDocs = await Value.find({
				groupNumber,
				organizations: {$in: organizations},
			});
	...
	userGetWorkbook: async (req, res, next) => {
		...
		try {
			...
			workbook = await Workbook.findById(
				workbook._id,
				'file name roAtts roCats'
			).populate('sheets');

			const pack = await Package.findOne(
				{groupNumber, name: packageName},
				'_id'
			);
			if (!pack)
				return next({
			...
			const populate = [];
			let values = await PackageValue.findOne({
				package: pack._id,
				groupNumber,
	...
	adminGetUserWorkbook: async (req, res, next) => {
		...
		try {
			...
			if (!pack.workbooks.find((wb) => wb.name === workbookName))
				return next({
					status: 400,
					message: `Workbook (${workbookName}) does not exist.`,
				});
	...
	userSaveWorkbook: async (req, res, next) => {
		...
		try {
			...
			let doc = await PackageValue.findOne({
				groupNumber,
				organization: organizations[0],
			});
			if (!doc)
			...
			return res.json({
				success: true,
				message: `Workbook (${workbook.name}) updated.`,
			});
		} catch (e) {
			next(e);
		}
	...
	userSubmitPackage: async (req, res, next) => {
		...
		const {pack, organizations} = await userGetPackageAndWorkbook(
			next,
			currentUserId,
			groupNumber,
			packageName
		...
		return res.json({
			success: true,
			message: `Package (${packageName}) submitted.`,
		});
	},
};
