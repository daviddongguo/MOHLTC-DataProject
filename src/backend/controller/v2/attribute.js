...
module.exports = {
	...
	addAttribute: async (req, res, next) => {
		...
		if (attribute) {
			...
			return res.json({
				success: true,
				message: `Attribute (${id}, ${name}) saved.`,
			});
		}
	},
	...
	deleteAttribute: async (req, res, next) => {
		...
		try {
			...
			res.json({
				success: true,
				result,
				message: `Removed ${result.deletedCount} attribute(s).`,
			});
		} catch (e) {
			next(e);
		}
	...
	attributeAssignGroups: async (req, res, next) => {
		...
		try {
			...
			res.json({
				success: true,
				result,
				message: `Updated ${result.modifiedCount} attribute(s).`,
			});
		} catch (e) {
			next(e);
		}
	...
	deleteAttributeGroup: async (req, res, next) => {
		...
		try {
			...
			res.json({
				success: true,
				message: `Removed attribute group (${_id}, ${doc.name})`,
			});
		} catch (e) {
			next(e);
		}
