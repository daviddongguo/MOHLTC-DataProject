...
module.exports = {
	adminGetAllWorkbooks: (req, res, next) => {
		...
		Workbook.find({groupNumber: groupNumber}, 'name', (err, workbooks) => {
			...
			return res.status(200).json({success: true, workbooks: workbooks});
		});
	},
	...
	saveWorkbookAdmin: async (req, res, next) => {
		...
		try {
			...
			if (oldWorkbook) {
				...
				oldSheetIds.forEach((_id) =>
					sheetOp.push({deleteOne: {filter: {_id}}})
				);
			}
			...
			return res.json({
				success: true,
				message: oldWorkbook
					? `Workbook (${workbook.name}) updated.`
	...
	adminGetWorkbook: async (req, res, next) => {
		...
		try {
			...
			if (!workbook) {
				return res
					.status(404)
					.json({success: false, message: '{name} does not exist.'});
			}
			// return next({status: 404, message: 'Workbook does not exist.'});
			return res.status(200).json({success: true, workbook});
		} catch (e) {
			next(e);
		}
	},
