...
module.exports = {
		...
		if (registerResponse == 'approve') {
			RegisterRequest.findOne({username: username}, (err, user) => {
				...
				User.register(newUser, user.password, (err, user) => {
					...
					sendMail.sendRegisterSuccessEmail(
						user.email,
						user.password,
						(info) => {
							RegisterRequest.findOneAndDelete({username: username}, function (
								err
							) {
								if (err) {
									return res.status(501).json({success: false, message: err});
	...
	delete_user: (req, res, next) => {
		...
		User.deleteOne({username: req.body.username}, (err) => {
			...
			return res.json({
				success: true,
				message: 'The user ' + req.body.username + ' has been deleted!',
			});
		});
	},
	admin_get_one_user_by_name: async (req, res, next) => {
		if (!checkPermission(req)) {
			return res
				.status(403)
				.json({success: false, message: error.api.NO_PERMISSION});
