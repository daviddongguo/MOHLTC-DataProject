...
export default function CreatePackage(props) {
  ...
  useEffect(() => {
    if (params.mode === "admin") {
      adminGetPackages().then((packages) =>
        setValues((values) => ({ ...values, packages }))
      );
    } else if (params.mode === "user") {
      userGetPackages().then((packages) =>
        setValues((values) => ({ ...values, packages }))
      );
    }
  }, [params.mode]);
  ...
  const onOpen = (name, pack) => (e) => {
    ...
    } else props.history.push("/packages/" + name);
  };

  const closeDialog = () =>
    setValues((values) => ({ ...values, dialog: false, selectedName: null }));
  const closePicker = () =>
    setValues((values) => ({ ...values, picker: null }));
  const openDialog = (name) =>
    setValues((values) => ({ ...values, dialog: true, selectedName: name }));
  ...
  const openPicker = (pack, anchorEl) => {
    ...
    if (organizations.length === 0) {
      ...
      props.history.push(
        "/admin/packages/" + pack.name + "/" + organizations[0]
      );
    } else {
      setValues((values) => ({
        ...values,
  ...
  const handleConfirmDelete = () => {
    ...
    if (params.mode === "admin") {
      adminDeletePackage(name)
        .then((res) => {
          closeDialog();
          setValues((values) => ({
            ...values,
            packages: values.packages.filter((p) => p.name !== name),
          }));
  ...
  const dialog = () => {
    return (
      <Dialog open={values.dialog} keepMounted onClose={closeDialog}>
        <DialogTitle>{"Confirm delete ?"}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete{" "}
            <strong>{values.selectedName}</strong>? <br />
  ...
  const handlePick = (org) => {
    props.history.push(
      "/admin/packages/" + values.pickedPackage.name + "/" + org
    );
  };
