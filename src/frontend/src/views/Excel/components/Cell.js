...
class Cell extends PureComponent {
  ...
  static getFontStyles(cell, rowHeight, style = {}) {
    ...
    if (cell.getStyle("strikethrough")) {
      ...
      else style.textDecoration = "line-through";
    }
  ...
  static getCellStyles(cell) {
    ...
    if (topBorder) {
      ...
      style.borderBottom = `${
        borderStyle2Width[topBorder.style]
      }px solid #${color}`;
    }
    ...
    if (leftBorder) {
      ...
      style.borderRight = `${
        borderStyle2Width[leftBorder.style]
      }px solid #${color}`;
    }
    ...
    const rightBorder =
      cell.getStyle("rightBorder") ||
      (bottomRightCell ? bottomRightCell.getStyle("rightBorder") : undefined);
    const bottomBorder =
      cell.getStyle("bottomBorder") ||
      (bottomRightCell ? bottomRightCell.getStyle("bottomBorder") : undefined);
    if (rightBorder) {
      const color = colorToRgb(rightBorder.color) || "000";
      style.borderRight = `${
        borderStyle2Width[rightBorder.style]
      }px solid #${color}`;
    }
    if (bottomBorder) {
      const color = colorToRgb(bottomBorder.color) || "000";
      style.borderBottom = `${
        borderStyle2Width[bottomBorder.style]
      }px solid #${color}`;
    }

    // horizontalAlignment
    const horizontalAlignment =
      supported.horizontalAlignment[cell.getStyle("horizontalAlignment")];
    ...
    const verticalAlignment =
      supported.verticalAlignment[cell.getStyle("verticalAlignment")];
    ...
    if (merged) {
      ...
      if (
        merged.from.row === cell.rowNumber() &&
        merged.from.col === cell.columnNumber()
      ) {
  ...
  onMouseUp = () => {
    if (mouse.mouseDown) {
      ...
      if (
        mouse.movementTarget[0] === "x" &&
        mouse.movementTarget[1] > 0 &&
        mouse.movement != null
      ) {
  ...
  onMouseMoveX = (e) => {
    if (mouse.mouseDown) {
      ...
      if (mouse.movementTarget[1] > 0 && mouse.movement != null) {
        ...
        const width =
          mouse.movement / 9.69 +
          (col.width() == null ? 80 / 9.69 : col.width());
        col.width(width >= 0 ? width : 0);
        excel.renderCurrentSheet();
      }
    }
  ...
  onMouseMoveY = (e) => {
    if (mouse.mouseDown) {
      ...
      if (mouse.movementTarget[1] > 0 && mouse.movement != null) {
        ...
        const height =
          mouse.movement / 1.666 +
          (row.height() == null ? 24 / 1.666 : row.height());
        row.height(height >= 0 ? height : 0);
        excel.renderCurrentSheet();
      }
    }
  ...
  renderColumnHeader(index, style) {
    ...
    if (!ref) ref = Cell.colHeaders[index] = React.createRef();
    ...
    return (
      ...
      >
        ...
        <span
          ...
          onMouseDown={this.onMouseDownX(index)}
        />
      </div>
    );
  }
  ...
  renderRowHeader(index, style) {
    ...
    if (!ref) ref = Cell.rowHeaders[index] = React.createRef();
    ...
    return (
      ...
      >
        ...
        <span
          ...
          onMouseDown={this.onMouseDownY(index)}
        />
      </div>
    );
  }
    ...
    return location ? (
      <div
        ...
        onClick={() => {
          ...
          if (sheet) excel.switchSheet(sheet);
        }}
      >
        {value}
      </div>
      ...
      <div
        style={innerStyle}
        onClick={() => window.open(cell.hyperlink(), "_blank")}
      >
        {value}
      </div>
    );
  ...
  renderNormalCell(value) {
    return <span style={{ pointerEvents: "none" }}>{value}</span>;
  }
  ...
  render() {
    ...
    if (dataValidation && dataValidation.type === "list") {
      innerContent = this.renderDataValidation(
        dataValidation,
        cell,
        value,
        mergedStyle
    ...
    return (
      <div
        ...
        tabIndex={0}
        onContextMenu={(e) =>
          onContextMenu(rowIndex, columnIndex, mergedStyle, e)
        }
        onKeyDown={(e) => onKeyDown(rowIndex, columnIndex, mergedStyle, e)}
        onDoubleClick={(e) =>
          onMouseDoubleClick(rowIndex, columnIndex, mergedStyle, e)
        }
        onMouseDown={(e) => onMouseDown(rowIndex, columnIndex, mergedStyle, e)}
        onPointerEnter={(e) =>
          onMouseOver(rowIndex, columnIndex, mergedStyle, e)
        }
