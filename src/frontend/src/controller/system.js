...
export async function updateOrganization({ name, users, managers, types }) {
  const response = await axios.put(
    config.server + "/api/v2/organizations",
    { name, users, managers, types },
    axiosConfig
  );
  if (check(response)) {
    return response.data;
...
export async function deleteOrganization(name) {
  const response = await axios.delete(
    config.server + "/api/v2/organizations/" + name,
    axiosConfig
  );
  if (check(response)) {
    return response.data;
...
export async function deleteOrganizationTypes(name) {
  const response = await axios.delete(
    config.server + "/api/v2/orgtypes/" + name,
    axiosConfig
  );
  if (check(response)) {
    return response.data;
